/* =====================================================
   MedConnect — Admin cloud-only patch
   -----------------------------------------------------
   Désactive l’accès administrateur local mc_admin_config.
   L’administrateur doit se connecter avec Firebase Auth
   et un profil Firestore users/{uid} role=admin.
   ===================================================== */
(function () {
  ...
  function saveAdminSession(profile, uid, email) {
    const session = {
      ...profile,
      uid,
      authUid: uid,
  ...
  async function login(event) {
    ...
    try {
      ...
      const session = saveAdminSession(profile, uid, email);
      App.closeModal();
