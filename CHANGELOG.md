...
- **Bouton « Accès Administrateur » enfin verrouillé pendant la connexion** (`js/firebase-config.js`, `MedConnectAdminCloud.login`). Cette connexion cloud **remplace** `Auth._doAdmin` (patch `adminCloudOnlyPatch`) : c'est elle qui est réellement câblée au bouton, mais elle n'avait jamais reçu le durcissement v2.9.43 (le test ne vérifiait que le `_doAdmin` masqué de `auth.js`). Elle porte désormais le même contrat : **verrou de module** (un seul essai simultané), **fermeture du clavier**, **état de chargement** sur le bouton (`App.setBtnLoading`), **timeout 15 s** (`App.withTimeout`) sur `signInWithEmailAndPassword` **et** la lecture du rôle, message clair au délai dépassé, verrou/chargement relâchés dans `finally`.
- **Tests** : `tests/fresh-device-login-rehydration-v2945.test.js` (10 cas — helper borné/non-fuite, ordre attente→`authLookup` dans auth.js et db.js, repli patient, e-mail déterministe, écoutes admin post-login, **durcissement de la connexion admin cloud + son câblage sur `_doAdmin`**, non-régression App Check/règles).
...
- **Connexions fiables dès le premier appui — desktop ET mobile.** Le même durcissement s'applique à **toutes** les connexions : administrateur, professionnel (médecin/infirmier/pharmacien), patient (mobile) et établissement (desktop). Verrou de module (un seul essai simultané, indépendant de l'état du bouton), **timeout de 15 s** sur les appels Firebase (une connexion qui pend ne fige plus le bouton — cause des « plusieurs appuis sans effet »), indicateur de chargement unifié (`App.setBtnLoading`) et helper partagé `App.withTimeout`, messages d'erreur distincts (délai vs identifiants/droits), **fermeture du clavier** avant la connexion. Pour l'admin, le **rôle est vérifié dans Firestore** avant d'ouvrir le tableau de bord. Verrouillé par `tests/login-hardening-v2943.test.js`.
...
- **Outbox multi-utilisateur sûre (poste partagé).** Chaque opération mémorise l'`ownerAuthUid` Firebase réel à la mise en file ; au rejeu, une opération d'un autre auteur est mise en **quarantaine** (jamais rejouée sous le mauvais compte, jamais supprimée) et attend le retour de son auteur. L'export de diagnostic devient **metadata-only** (plus aucun payload : ni nom, MC, téléphone, diagnostic, médicament, message, PIN, code).
- **Migration de production `scripts/migrate-production-v2942.mjs`.** Rétablit la cohérence `establishmentId` ⇆ `hospital_id` (champ manquant recopié, jamais d'écrasement, conflit/ambigu signalés pour traitement manuel) et (re)crée les entrées `patient_directory` manquantes. Strictement additif : ne supprime rien, ne réinitialise rien, ne modifie aucun identifiant ; journal avant/après, `--dry-run` par défaut, `--apply` gardé par `--i-have-a-backup`, `--limit`, `--resume-from`, `--collection`.
- **Recherche patient bornée à l'établissement** (`searchPatients(q, establishmentIds?)`) : aucune fiche hors des établissements de l'agent ne remonte, même si elle traîne encore en cache (rétro-compatible sans le 2ᵉ paramètre).
- **Pharmacie — réconciliation des ventes hors ligne.** Une vente hors ligne est rejouée à la reconnexion par une **transaction** qui relit le stock réel et le décrémente relativement (jamais d'écrasement absolu périmé), refuse et **quarantaine** si le stock est devenu insuffisant, et reste **idempotente**. La garantie « pas de survente » couvre désormais en ligne **et** hors ligne.
- **Messagerie fiable** (verrou de non-régression) : aucun faux « envoyé » avant confirmation serveur, une seule écriture ciblée `mc_messages/{mid}` (jamais de copie `notifications`), statut modifiable sans jamais altérer le contenu/l'expéditeur/le destinataire.
- **Sécurité serveur (chantiers 6/7, préparation).** `functions/` : `authLookup`, `claimPatientAccount`, `setVerifiedClientType` (2ᵉ génération, App Check exigé, config économe). Câblage client en **feature-detection** : la voie serveur est tentée puis retombe proprement sur la lecture actuelle tant que les fonctions ne sont pas déployées — **rien ne casse avant déploiement**. La fermeture de la lecture publique de `mc_accounts` est une étape **gatée** (après déploiement + validation des fonctions).
- **Durcissement production (chantier 12).** En-têtes de sécurité, export metadata-only, logs sans contenu clinique, App Check activé. Enforcement App Check et CSP bloquante = étapes go-live gatées (staging d'abord).

- **Système de notifications médicales multi-appareils** (nouveau, 7 phases). Architecture 4 niveaux : événement métier canonique → notification Firestore (source de vérité) → résolution des appareils par Cloud Functions → envoi au fournisseur adapté. Quatre collections (`notifications`, `pushRegistrations`, `notificationPreferences`, `notificationDeliveries`), déclencheurs sur transitions RÉELLES (idempotence par `deduplicationKey`), file d'envoi avec retry/backoff + invalidation 404/410 + cron de nettoyage, centre de notifications in-app (cloche universelle, badge synchronisé, préférences), et push natif FCM Web / Web Push iOS / FCM Android / Electron. **Aucune donnée médicale ne transite en push** (payload `notificationId/category/priority/deepLink` seulement) ; le client relit la notification autorisée depuis Firestore après Auth + App Check + droits. Voir `docs/NOTIFICATIONS_v2.9.42.md` (étapes d'activation : VAPID, `google-services.json`, redeploy).

- **Fermeture de la lecture publique de `mc_accounts`** : la lecture n'est plus `if true` mais réservée au titulaire (`docId`/`authUid` == uid) ou à l'admin. Les résolutions inter-comptes passent par des Cloud Functions de confiance (`authLookup`, nouvelle `checkAgentDuplicate` pour la détection de doublon à l'inscription) ; le listener collection-entière `mc_accounts` n'est conservé que pour l'admin. Client à repli lecture directe pour une transition sans coupure. **Règle DEPLOY-GATED** : déployer les fonctions AVANT les règles (sinon la connexion casse).

- **CSP passée en mode bloquant** (`Content-Security-Policy` dans `firebase.json`), après audit complet des ressources réellement chargées (Firebase SDK, reCAPTCHA/App Check, Leaflet, Google Fonts, QR codes, Claude API, EuropePMC, Overpass) : ajout des hôtes manquants (`fonts.googleapis.com`/`fonts.gstatic.com`, `api.qrserver.com`, `api.anthropic.com`, `www.ebi.ac.uk`, `overpass-api.de`) et durcissement (`frame-ancestors 'none'`, `worker-src`/`manifest-src 'self'`, `object-src 'none'`, `base-uri`/`form-action 'self'`). `script-src` conserve `'unsafe-inline'` (l'UI repose sur des gestionnaires `onclick=` inline). Contrat verrouillé par `tests/csp-hosting-headers-v2942.test.js`. Script de déploiement ordonné à gates : `scripts/deploy-ordered-v2942.sh` (fonctions → règles → hosting, avec validation manuelle entre chaque étape).

**Coût Blaze** réduit par les leviers déjà acquis (listeners bornés par établissement au lieu de collection-entière, arrêt des doubles écritures, persistance hors-ligne, `minInstances=0` sur les fonctions). La fermeture de `mc_accounts` supprime en plus un listener collection-entière par session non-admin.

Tests : `outbox-multiuser-v2942`, `migrate-production-v2942`, `patient-search-scoping-v2942`, `pharmacy-offline-sale-reconcile-v2942`, `messaging-reliability-v2942`, `cloud-functions-wiring-v2942`, `production-security-v2942` (+ règles). Version **2.9.42** (build 2026.07.23.2, versionCode 43, cache `medconnect-v4.43`). Miroirs Android resynchronisés octet pour octet.

**Verdict : RELEASE BLOCKED** tant que les Cloud Functions ne sont pas déployées et validées, et que la fermeture de la lecture publique de `mc_accounts` + l'enforcement App Check/CSP ne sont pas confirmés en staging. Voir `docs/PRODUCTION_READINESS_v2.9.42.md` et `docs/DEPLOYMENT_v2.9.42.md`.
